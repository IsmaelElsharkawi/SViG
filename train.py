...
parser.add_argument("--start-thresh", type=float, help="starting threshold for the aggregation")
parser.add_argument("--dec", type=float, help="decrement in each layer")
...
def main():
    ...
    model = create_model(
        ...
        starting_threshold = args.start_thresh, 
        decrement = args.dec,
        global_pool=args.gp,
        bn_tf=args.bn_tf,
    ...
    if args.pretrain_path is not None:
        ...
        if args.model_ema:
            model.load_state_dict(state_dict['state_dict_ema'], strict=True)
        else:
    ...
    model.train()
    _logger.info('model flops: {0} input_size: {1}'.format(macs, input_size))
    ##########################################
    ...
    if args.local_rank == 0:
        ...
        _logger.info("Current output directory: {}".format(output_dir))
